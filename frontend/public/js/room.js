document.addEventListener("DOMContentLoaded", async () => {
  ...
  // ジャンル一覧をDBから取得して反映
  try {
    const response = await fetch("https://procon-e8vw.onrender.com/api/genres");
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      // テーマランダム取得
      const themeRes = await fetch(`https://procon-e8vw.onrender.com/api/theme?genre_id=${genre_id}`);
      ...
      for (const name of playerNames) {
        const playerRes = await fetch("https://procon-e8vw.onrender.com/api/players", {
          method: "POST",
      ...
      // 人狼決定
      console.log("人狼決定");
      const wolfRes = await fetch("https://procon-e8vw.onrender.com/api/wolf", { method: "POST" });
      ...
      // テーマ割り当て
      console.log("テーマ割り当て");
      const postThemeRes = await fetch("https://procon-e8vw.onrender.com/api/post-theme", { method: "POST" });
